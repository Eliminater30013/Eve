...
function Player:calculateJumps()
    ...
    if self.map:tileAt(self.x, self.y).id == FLAG_BOTTOM or 
        self.map:tileAt(self.x, self.y).id == FLAG_MID or self.map:tileAt(self.x, self.y).id  == FLAG_POLE or 
        self.map:tileAt(self.x, self.y).id  == FLAG_START then
        self.sounds['coin']:play()
        MAP_INIT = false -- music
        MAP_COUNT = MAP_COUNT + 1
        POINTS = POINTS + 10
        self.map:init() -- reinit map AND increase MAP COUNT
    elseif self.map:tileAt(self.x + self.width - 1, self.y).id == FLAG_BOTTOM or self.map:tileAt(self.x + self.width - 1, self.y).id == FLAG_MID or self.map:tileAt(self.x + self.width - 1, self.y).id  == FLAG_POLE  or self.map:tileAt(self.x + self.width - 1, self.y).id  == FLAG_START then
        self.sounds['coin']:play()
        MAP_INIT = false -- music
        MAP_COUNT = MAP_COUNT + 1
        POINTS = POINTS + 10
        self.map:init()
    elseif self.y + 225 > self.map.mapHeightPixels then
        self.sounds['death']:play()
        MAP_INIT = false -- music
        local score = MAP_COUNT
        local points_earned = POINTS
        if score > LEVEL_REACHED then
            LEVEL_REACHED = score
        end
        if points_earned > H_POINTS then
            H_POINTS = points_earned
        end
        POINTS = 0
        MAP_COUNT = 1
        DEAD = DEAD + 1
